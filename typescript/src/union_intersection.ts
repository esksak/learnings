...
// ユニオン型
// age propは number | undefined型
type User = Human | Animal;

// インターセクション型
// age propは number & undefined = never型
type SuperHuman = Animal & Human;
...
// ユニオン型で構成された関数の引数には、インターセクション型を入れる必要がある
type UnionFunc = 
  ((animal: Animal) => string)
  | ((human: Human) => string)
  
function getSpecies(anilmal: Animal) {
...
const randFunc: UnionFunc  = Math.random() < 0.5 ? getSpecies : getName;
...
function getNamesIfAllHuman(users: readonly User[]): string[] | undefined {
    if (users.every(u => u.tag === "human")) {
        ...
        return (users as Human[]).map(u => u.name);
    }
}
...
function isHuman2(value: any): asserts value is Human {
    if (value == null) {
